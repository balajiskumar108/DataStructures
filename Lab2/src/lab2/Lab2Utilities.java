    ...
    * </p>
        ...
        for(int i = 1; i < inputStr.length(); i++) {
        	if(inputStr.charAt(i) == inputStr.charAt(i - 1)) {
        		count++;
        		 ...
        		 compressed +=  inputStr.charAt(i - 1) + ""+ count;
        		 count = 1;
        	}
        }
        
        compressed += inputStr.charAt(inputStr.length()-1) + "" + count;
        return compressed;
    }
	...
	public static String binaryXor(short value1 , short value2) {
		...
		for(int i = 0; i < 16; i++) {
			...
			int xorBit = (bit1 == bit2) ? 0: 1;
			
			binaryResult = xorBit + binaryResult;
			
			value1 = (short) (value1/2);
			value2 = (short) (value2/2);
		}
		return "XOR of two values is [" + binaryResult + "]";
	...
	public static String isPalindromeInt(int n) {
		...
		if(n < 10) {
			return String.format("The %d is not a palindrome", n);
		}
